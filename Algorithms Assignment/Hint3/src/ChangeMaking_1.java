...
public class ChangeMaking_1 {
	...
	public int getCandidate(int changeGenerated, 
		...
		while((i<length)&&(res == -1)){
			int element = coinValues.getElement(i);
			if(discarded.getElement(i)==0){
				res = i;
				discarded.addElement(i,element);
				changeGenerated = changeGenerated+element;
			}
			else if(discarded.getElement(i)>=element && element>=0){
				res = i;
				changeGenerated = changeGenerated+element;
			}
			i++;
	...
	public boolean isValid(MyList<Integer> coinValues,
		...
		//SET OF OPS
		//-----------------------------
		//check if the element in coin values at the index given plus the change previously
		//generated is less than or equal to the amount required
